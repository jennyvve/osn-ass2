...
/* Each level of the page table contains 2^(...) entries; each entry
 * is ...
 */
...
struct __attribute__((__packed__)) TableEntry {
    ...
    uint64_t
        ppn : 44; /* Physical page number (page offset not stored). */

    uint16_t reserved2 : 10;
};

const static int entries = pageSize / sizeof(TableEntry);
const static int vpnMask = entries - 1;
...
class MMUDriver : public ::MMUDriver {
   ...
   public:
    ...
    virtual void releasePageTableInternal(TableEntry *table);
    virtual uintptr_t getPageTable(const uint64_t PID) override;

    virtual void setMapping(const uint64_t PID, uintptr_t vAddr,
                            PhysPage &pPage) override;

    virtual uint64_t getBytesAllocated(void) const override;

    /* Disallow objects from being copied, because it has a pointer
     * member. */
