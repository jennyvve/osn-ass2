...
MMU::MMU() : root(0x0), pageFaultHandler(), tlb(*this, TLBEntries) {}
...
MMU::~MMU() {
    ...
    std::cerr << std::dec << std::endl
              ...
              << "# hits: " << stats.hits << " (" << pct << "%)"
              << std::endl
              << "# evictions due to add: " << stats.addEvictions
              << std::endl
              << "# flushes: " << stats.flushes << std::endl
              << "# evictions due to flush: " << stats.flushEvictions
              << std::endl;
}

/* Initialize the MMU with a given page fault handler @fn.
 */
void MMU::initialize(PageFaultFunction fn) { pageFaultHandler = fn; }

/* Set the current page table root to @_root.
 */
void MMU::setPageTablePointer(const uintptr_t _root) { root = _root; }
...
void MMU::processMemAccess(const MemAccess &access) {
    ...
    if (LogMemoryAccesses) {
        std::cerr << "MMU: translated virtual " << std::hex
                  << std::showbase << access.addr << " to physical "
                  << pAddr << std::endl;
    }
}

/* Returns the physical address for a memory access @access to page
 * @pPage.
 */
uint64_t MMU::makePhysicalAddr(const MemAccess &access,
                               const uint64_t pPage) {
...
bool MMU::getTranslation(const MemAccess &access, uint64_t &pAddr) {
    /* Strip off (zero out) unused sign-extension bits in virtual
     * address */
    const uint64_t vAddr =
                       access.addr & ((1UL << getAddressBits()) - 1),
    ...
    if (EnableTLB && getTLB().lookup(vPage, pPage)) {
        pAddr = makePhysicalAddr(access, pPage);
        return true;
    } else if (performTranslation(vPage, pPage, isWrite)) {
        pAddr = makePhysicalAddr(access, pPage);
        if (EnableTLB) {
            getTLB().add(vPage, pPage);
        }
        return true;
    }
